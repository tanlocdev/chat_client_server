...
namespace Server
{
    public partial class Server : Form
    {
        ...
        private void Log(string msg = "") // xóa nhật ký nếu thông báo không được cung cấp hoặc trống
        {
            if (!exit)
            {
            ...
            if (!exit)
            {
                startButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg("Server đã bắt đầu"));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg("Server đã dừng lại"));
                    }
                });
            }
        }
        ...
        private void AddToGrid(long id, string name)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    totalLabel.Text = string.Format("Tổng số máy khách: {0}", clientsDataGridView.Rows.Count);
                });
            }
        }
        ...
        private void StartButton_Click(object sender, EventArgs e)
        {
            ...
            else if (listener == null || !listener.IsAlive)
            {
                ...
                {
                    error = true;
                    Log(SystemMsg("Địa chỉ là bắt buộc"));
                }
                else
                {
                    try
                    ...
                    {
                        error = true;
                        Log(SystemMsg("Địa chỉ không hợp lệ"));
                    }
                }
                int port = -1;
                if (number.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Port là bắt buộc"));
                }
                else if (!int.TryParse(number, out port))
                {
                    error = true;
                    Log(SystemMsg("Số port không hợp lệ"));
                }
                else if (port < 0 || port > 65535)
                {
                    error = true;
                    Log(SystemMsg("Số cổng nằm ngoài phạm vi"));
                }
                if (username.Length < 1)
                {
                    error = true;
                    Log(SystemMsg("Tên người dùng là bắt buộc"));
                }
                if (!error)
                {
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sendTextBox.Text.Length > 0)
                {
                    ...
                    sendTextBox.Clear();
                    Log(string.Format("{0} (Bạn): {1}", usernameTextBox.Text.Trim(), msg));
